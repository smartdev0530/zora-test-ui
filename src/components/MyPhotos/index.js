...
export default class MyPhotos extends Component {
  ...
  setAskPrice = async () => {
    try {
        ...
        const askResult = await mediaInstance.methods.setAsk(this.state.tokenId, 
            {
                amount : weiValue, 
                currency : process.env.REACT_APP_WETH_ADDRESS
            }
            )
  ...
  cancelAuction = async (auctionId) => {
    try {
      ...
      const result = await auctionHouseInstance.methods.cancel(auctionId).send({from : currentAccount})
      console.log('auction end result', result)
            ...
            return (
              <div className={styles.widgets} style={{display : "flex", flexDirection : "row"}} key = {key}>
                ...
                >
                    ...
                    {photo.status ==='IN_PROGRESS' && expiresAt == 0 &&
                    (
                      <Button
                          size={"medium"}
                          width={1}
                          ...
                          onClick={() => this.endAuction(photo.auctionId)}
                      >
                          End Auction
                      </Button>
                    )
                    ...
                    (
                      <Button
                          ...
                          onClick={() => this.cancelAuction(photo.auctionId)}
                      >
                          Cancel Auction
                      </Button>
                    )
