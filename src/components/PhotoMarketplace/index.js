...
export default class PhotoMarketplace extends Component {
  ...
  buyNow = async () => {
    try {
        ...
        const buyResult = await mediaInstance.methods.setBid(this.state.tokenId, {
            amount : this.state.askValue, 
            currency : this.state.askCurrency, 
  ...
  render() {
    ...
    let expiresAt = 0;
    if (auction && auction.expiresAt) expiresAt = (new Date(auction.expiresAt)).getTime();
    expiresAt -= (new Date()).getTime();
    if (expiresAt < 0) expiresAt = 0;
    expiresAt /= 1000;
    
    const reservePrice = (web3 && auction && auction.reservePrice) ? web3.utils.fromWei(auction.reservePrice) : 0.0
    const highestBid = (web3 && auction && auction.lastBidAmount) ? web3.utils.fromWei(auction.lastBidAmount) : 0.0
      ...
      <div className={styles.contracts}>
        ...
        {photo &&
            ...
            >
                ...
                <p>Ask currency : {this.state.askCurrency}</p>
                ...
                <Button
                    ...
                    onClick={() => this.buyNow()}
                    disabled = {this.state.askCurrency.length == 0 || this.state.askCurrency == '0x0000000000000000000000000000000000000000'}
                >
                    Buy It Now
                </Button>
